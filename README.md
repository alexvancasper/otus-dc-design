...
## Проектирование сети Underlay
Для просты конфигурации и придерживания принципа "сделать все максимально просто", 
Underlay будет работать на основе eBGP протокола.
...
Между Spine свитчами нужно обеспечить обмен префиксов для этого необходимо включить `as-loop-in 1` или `as-override, чтобы leaf свитч принимая префикс от spine-2 и анонсируя их в сторону spine-1, spine-1 их принимал.
т.е. отключаем встроенную в BGP механизм предотвращения петель, ровно на одно вхождение.
...
iBGP протокол требует full-mesh связности между всеми узлами сети, либо использовать route-reflector который упростит задачу масштабируемости и эксплуатции.
iBGP AS 4210000001
